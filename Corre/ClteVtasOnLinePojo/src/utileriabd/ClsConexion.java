...
// ---------------------------------------------------------------------------    
    ...
    public boolean conectate( String unUID, String unPwd )
    {
        ...
        try
        {
            CodeSource codeSource = ClsConexion.class.getProtectionDomain().getCodeSource();
            ...
            //###################NUEVO###############################
            con = DriverManager.getConnection("jdbc:ucanaccess://" + jarFile +"/../../" + this.strNomDB);  
            //#######################################################
...
// ---------------------------------------------------------------------------    
    ...
    //#####################NUEVO#############################
    public java.sql.ResultSet obtenCostoUnitario(String clave)
    {
        return obtenRegSelect("Select UnitPr FROM Products WHERE StockCode='"+clave+"'");
    }
    
     public java.sql.ResultSet obtenDescripcion(String clave)
    {
        return obtenRegSelect("Select ProdDescription FROM Products WHERE StockCode='"+clave+"'");
    }
    //#######################################################
// ---------------------------------------------------------------------------    
...
// ---------------------------------------------------------------------------    
  ...
  // ---------------------------------------------------------------------------
  //#####################NUEVO#############################
  public Pedido updateSumaCantidad(Pedido pedido){
          ...
          if(c.conectado())
          {
            ...
            while(iterator.hasNext()){
                ...
                cantidad = (int)mentry.getValue();
                pedido.sumarCantidad(cantidad);
                rs = c.obtenCostoUnitario(clave);
                rs2 = c.obtenDescripcion(clave);
              try {
                  rs.next();
                  costo = rs.getDouble(1); 
                  rs2.next();
                  desc = rs2.getString(1);
                  pedido.agregarArreglos(cantidad, costo,desc);
                  pedido.sumarTotal(costo, cantidad);
                ...
                switch( intPrueba[i] )
                {
                 ...
                 case 5:
                   ...
                   case 11:
                       ...
                       break;
    //#####################NUEVO#############################
                   case 12:  // lista productos
                        System.out.println("Empece");
                        r = c.obtenRS("Invoice_Headers");
                        System.out.println("Terminé");
                        if( r != null )
                        {
                            System.out.println("Empece a imprmir");
                          System.out.println(c.impRS(r));
                        }
                        else
                            System.out.println(
